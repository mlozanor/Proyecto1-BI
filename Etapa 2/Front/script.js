document.addEventListener('DOMContentLoaded', function() {
    ...
    // Exportar modelo - MODIFICADO
    exportModelButton.addEventListener('click', async () => {
        try {
            // Intentar primero con el endpoint original con timeout extendido
            const response = await fetchWithTimeout('http://localhost:8000/export-model/', {}, 60000);
            
            if (!response.ok) {
                throw new Error(`Error en la respuesta: ${response.status}`);
            }
            
            // Si es una respuesta de archivo, descargar directamente
            ...
            if (contentType && contentType.includes('application/json')) {
                ...
                let filename = `modelo_noticias_${new Date().toISOString().slice(0, 10)}.json`;
                ...
                // Si no es un archivo, intentar procesar como JSON
            ...
            alert(`Error al exportar el modelo: ${err.message}. Intenta recargar la página y asegúrate de que el servidor esté en ejecución.`);
        }
    });
    
    // Exportar modelo recién entrenado - MODIFICADO
    ...
    function downloadModel() {
        fetch("http://localhost:8000/download-model/")
            .then(response => {
                if (!response.ok) {
                    throw new Error(`Error al descargar el modelo: ${response.status}`);
                }
            ...
            .then(blob => {
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = "modelo_entrenado.pkl";
                document.body.appendChild(a);
                a.click();
    ...
    // Función para descargar la información del modelo
    function downloadModelInfo() {
        fetch("http://localhost:8000/download-model-info/")
            .then(response => {
                if (!response.ok) {
                    throw new Error(`Error al descargar la información del modelo: ${response.status}`);
                }
                return response.blob();
            })
            .then(blob => {
            ...
            .catch(error => {
                console.error("Error al descargar la información del modelo:", error);
                alert(`Error al descargar la información del modelo: ${error.message}`);
            });
    }

    // Añadir botones de descarga al modal de información del modelo
    document.addEventListener("DOMContentLoaded", function() {
        // Verificar si ya existe el contenedor de botones
        const modelInfoContent = document.getElementById('model-info-content');
        
        if (modelInfoContent) {
            // Crear botones de descarga si no existen
            const downloadButtonsContainer = document.createElement('div');
            downloadButtonsContainer.id = 'download-buttons-container';
            downloadButtonsContainer.className = 'mt-4 flex space-x-2';
            downloadButtonsContainer.innerHTML = `
                <button onclick="downloadModel()" class="px-3 py-1 text-sm bg-purple-600 text-white rounded-md hover:bg-purple-700">
                    Descargar Modelo (.pkl)
                </button>
                <button onclick="downloadModelInfo()" class="px-3 py-1 text-sm bg-teal-600 text-white rounded-md hover:bg-teal-700">
                    Descargar Info Modelo (.json)
                </button>
            `;
            
            // Añadir los botones después de cargar la información del modelo
            const originalModelInfoButton = document.getElementById('model-info-button');
            if (originalModelInfoButton) {
                const originalClickHandler = originalModelInfoButton.onclick;
                
                originalModelInfoButton.onclick = function(e) {
                    // Llamar al manejador original si existe
                    if (originalClickHandler) {
                        originalClickHandler.call(this, e);
                    }
                    
                    // Añadir los botones de descarga después de un breve retraso
                    setTimeout(() => {
                        // Verificar si ya existen los botones
                        if (!document.getElementById('download-buttons-container')) {
                            // Añadir después del contenido de información del modelo
                            modelInfoContent.parentNode.insertBefore(
                                downloadButtonsContainer, 
                                modelInfoContent.nextSibling
                            );
                        }
                    }, 500);
                };
